...
<head>
  <meta charset="utf-8">
  <title>SLIM | Network Provider</title>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0" style="color: var(--text-dark); font-weight: 700;">Network Provider Management</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item">
                <a href="<?= base_url('portofolio')?>" class="breadcrumb-home">Home</a></li>
              <li class="breadcrumb-item active">Network Provider</li>
            </ol>
          </div>
        </div>
      </div>
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-submenu-compact" style="border-top: 3px solid var(--theme-yellow-primary);">
            ...
            <form id="mainFilterForm" action="<?= base_url('network_provider') ?>" method="get">
                        ...
                        <div class="col-md-4">
                            <div class="input-group">
                                ...
                                <div class="input-group-append">
                                    ...
                                    <a href="<?= base_url('network_provider') ?>" class="btn btn-secondary d-flex align-items-center">
                        ...
                        <table class="table table-striped table-bordered table-hover text-nowrap" style="width: 100%;">
                            <thead>
                                <tr class="bg-info">
                                    ...
                                    <?= render_th('Provider Name', 'provider_name', $opt_provider_name, $selected_filters) ?>
                                        ...
                                        <td class="text-center align-middle">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right shadow dropdown-operation-menu">
                                                    <button class="dropdown-item" type="button" onclick="window.location.href='<?= base_url('network_provider/audit/'.$row['provider_id']) ?>'">
                                                        <i class="fas fa-clipboard-list fa-fw text-primary mr-2"></i> Audit Trail
                                                    </button>

                                                    <?php if(isset($row['status']) && $row['status'] == 1): ?>
                                                        <button class="dropdown-item" type="button" onclick="editProvider('<?= $row['provider_id'] ?>', '<?= htmlspecialchars($row['provider_name'], ENT_QUOTES) ?>')">
                                                            <i class="fas fa-edit fa-fw text-warning mr-2"></i> Edit Data
                                                        </button>
                                                    
                                                        <button class="dropdown-item" type="button" onclick="confirmDelete('<?= $row['network_provider_id'] ?>')">
                                                            <i class="fas fa-power-off fa-fw text-danger mr-2"></i> Deactivate
                                                        </button>
                                                    <?php else: ?>
                                                        <button class="dropdown-item" type="button" onclick="confirmRestore('<?= $row['network_provider_id'] ?>')">
...
<div class="modal fade modal-submenu-compact" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
            ...
            <form action="<?= base_url('network_provider/save') ?>" method="post">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="network_id" id="network_id" style="width: 100%;" required>
...
<script>
    ...
    function confirmExport() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "<?= base_url('network_provider/export') ?>" + window.location.search;
            }
        })
    }
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            text: "Data provider akan dinonaktifkan.",
            icon: 'warning',
            input: 'text',
            inputLabel: 'Alasan:',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= base_url('network_provider/update_status') ?>",
                    type: "POST",
                    dataType: "JSON",
                    data: { 
                        id: id, 
    ...
    function confirmRestore(id) {
        Swal.fire({
            ...
            text: "Data provider ini akan dikembalikan ke daftar aktif.",
            icon: 'info',
            input: 'text',
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            Swal.fire({
                                ...
                                text: response.message,
                                confirmButtonText: 'OK',
                                customClass: { confirmButton: 'btn btn-theme-gradient px-4 mx-2' }
                            }).then(() => { location.reload(); });
                        } else {
                            Swal.fire({
                                icon: 'error',
    ...
    $(document).on('input', '#provider_name, #reason, input[name="keyword"], .filter-search-input, .swal2-popup input[type="text"], .swal2-popup textarea', function() {
        var el = $(this);
        var currentValue = el.val();
        var forbiddenChars;

        var isBracketAllowed = false;
        if (el.attr('id') === 'provider_name' || el.attr('name') === 'keyword' || el.hasClass('filter-search-input')) {
            isBracketAllowed = true;
        }

        if (isBracketAllowed) {
            // Izinkan: Huruf, Angka, Spasi, Titik, Koma, Strip, Underscore, dan Kurung ()
            forbiddenChars = /[^a-zA-Z0-9\s.,_\-()]/g;
        } else {
            // Standar (untuk Reason & Swal Pop-up): Tanpa tanda kurung
            forbiddenChars = /[^a-zA-Z0-9\s.,_\-]/g; 
        }
    ...
    $(document).on('paste', '#provider_name, #reason, input[name="keyword"], .filter-search-input, .swal2-popup input[type="text"], .swal2-popup textarea', function(e) {
        var el = $(this);
        // Ambil data teks dari clipboard
        var pasteData = (e.originalEvent || e).clipboardData.getData('text');
        var forbiddenChars;

        // 1. Tentukan apakah input ini boleh pakai tanda kurung ()
        var isBracketAllowed = false;
        if (el.attr('id') === 'provider_name' || el.attr('name') === 'keyword' || el.hasClass('filter-search-input')) {
            isBracketAllowed = true;
        }

        // 2. Set Regex sesuai aturan
        if (isBracketAllowed) {
            forbiddenChars = /[^a-zA-Z0-9\s.,_\-()]/g;
        } else {
            forbiddenChars = /[^a-zA-Z0-9\s.,_\-]/g; 
        ...
        // 3. Eksekusi blokir jika teks yang di-paste mengandung karakter terlarang
        if (forbiddenChars.test(pasteData)) {
            // Gagalkan proses paste secara total
            e.preventDefault();
            
            // Efek visual border merah berkedip sebagai peringatan
            el.css({
                'border-color': '#dc3545',
                'box-shadow': '0 0 0 0.2rem rgba(220, 53, 69, 0.25)'
            });
