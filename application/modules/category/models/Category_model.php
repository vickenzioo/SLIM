...
class Category_model extends CI_Model {
    ...
    public function count_all_categorys($keyword = null, $filters = []) {
        if($keyword) {
            $this->db->group_start();
        ...
        return $this->db->count_all_results('tbl_apps_category');
    }

    // 4. Get Paginated (Support Filter + ORDER BY CATEGORY NAME ASC)
    public function get_categorys_paginated($limit, $start, $keyword = null, $filters = []) {
        if($keyword) {
            $this->db->group_start();
            $this->db->like('category_name', $keyword);
            $this->db->or_like('standard_category', $keyword);
        ...
        $this->db->order_by('standard_category', 'DESC'); // Mengurutkan dari terbesar ke terkecil
        $this->db->order_by('category_name', 'ASC');
        
        if ($limit > 0) {
            $this->db->limit($limit, $start);
        }
        return $this->db->get('tbl_apps_category')->result_array();
    }
    ...
    public function get_dynamic_options($target_key, $current_filters = []) {
        ...
        $query = $this->db->get('tbl_apps_category');
    ...
    // 6. Get All for Export (Support Filter + ORDER BY CATEGORY NAME ASC)
    public function get_all_categorys($keyword = null, $filters = []) {
        if($keyword) {
            $this->db->group_start();
            $this->db->like('category_name', $keyword);
            $this->db->or_like('standard_category', $keyword);
    ...
    // --- CRUD Standard ---
    public function insert_category($data) { return $this->db->insert('tbl_apps_category', $data); }
    public function update_category($id, $data) { $this->db->where('category_id', $id); return $this->db->update('tbl_apps_category', $data); }
    public function delete_category($id) { $this->db->where('category_id', $id); return $this->db->delete('tbl_apps_category'); }
    
    public function check_duplicate_category($name, $id = null) {
        $this->db->where('category_name', $name);
        if($id) { $this->db->where('category_id !=', $id); }
        return $this->db->get('tbl_apps_category')->num_rows() > 0;
    }
    
    public function get_by_id($id) {
        return $this->db->get_where('tbl_apps_category', ['category_id' => $id])->row_array();
    }
	
	public function update_status($id, $status) {
        $this->db->where('category_id', $id);
        return $this->db->update('tbl_apps_category', ['status' => $status]);
    }
	
    public function get_audit_trail($id) {
        $this->db->where('table_name', 'tbl_apps_category'); 
