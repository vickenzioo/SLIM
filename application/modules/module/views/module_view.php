  ...
  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0" style="color: var(--text-dark); font-weight: 700;">Module Management</h1>
          </div>
          <div class="col-sm-6">
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="card" style="border-top: 3px solid var(--theme-yellow-primary);">
          <div class="card-body">
                <form id="mainFilterForm" action="<?= base_url('module') ?>" method="get">
                    ...
                    <div class="row align-items-center mb-3">
                        ...
                        <div class="col-md-4">
                            <div class="input-group">
                                <input type="text" name="keyword" class="form-control" placeholder="Search Module Name..." value="<?= isset($keyword) ? $keyword : '' ?>">
                                <div class="input-group-append">
                                    <button class="btn btn-default"><i class="fas fa-search"></i></button>
                                    <a href="<?= base_url('module') ?>" class="btn btn-secondary d-flex align-items-center">
                    ...
                    <table class="table table-striped table-bordered table-hover text-nowrap">
                        <thead>
                            <tr class="bg-info">
                                ...
                                ?>
                                <?= render_th('Module Name', 'module_name', $opt_module_name, $selected_filters) ?>
                        ...
                        <tbody>
                            <?php if(!empty($modules)): ?>
                                ...
                                <tr>
                                    <td class="text-center align-middle">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right shadow dropdown-operation-menu">
                                                <button class="dropdown-item" type="button" onclick="window.location.href='<?= base_url('module/audit/'.$m['module_id']) ?>'">
                                                    <i class="fas fa-clipboard-list fa-fw text-primary mr-2"></i> Audit Trail
                                                </button>
                                                <?php if($m['status'] == 1): ?>
                                                    <button class="dropdown-item" type="button" onclick="editData(<?= $m['module_id'] ?>, '<?= $m['module_name'] ?>')">
                                                        <i class="fas fa-edit fa-fw text-warning mr-2"></i> Edit Data
                                                    </button>
                                                    <button class="dropdown-item" type="button" onclick="confirmDelete(<?= $m['module_id'] ?>)">
                                                        <i class="fas fa-power-off fa-fw text-danger mr-2"></i> Deactivate
                                                    </button>
                                                <?php else: ?>
                                                    <button class="dropdown-item" type="button" onclick="confirmRestore(<?= $m['module_id'] ?>)">
                                    ...
                                    <td class="align-middle"><?= $m['module_name'] ?></td>
                                    <td class="text-center align-middle">
                                        <?php 
...
<div class="modal fade" id="modalModule" tabindex="-1" role="dialog">
      ...
      <form action="<?= base_url('module/save') ?>" method="post">
          <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="module_name" id="module_name" class="form-control" required placeholder="Enter Module Name">
            </div>
...
<script>
    ...
    function confirmExport() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "<?= base_url('module/export') ?>" + window.location.search;
            }
        })
    }
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= base_url('module/update_status') ?>",
                    type: "POST",
    ...
    function updateStatus(id, status, reason) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    Swal.fire({ icon: 'success', title: 'Berhasil', text: response.message, customClass: { confirmButton: 'btn btn-theme-gradient px-4' } }).then(() => { location.reload(); });
                } else {
    ...
    <?php if($this->session->flashdata('success')): ?>
        Swal.fire({ icon: 'success', title: 'Success', text: '<?= $this->session->flashdata('success') ?>', customClass: { confirmButton: 'btn btn-theme-gradient px-4' } });
    <?php endif; ?>

    <?php if($this->session->flashdata('error')): ?>
        Swal.fire({ icon: 'error', title: 'Gagal!', text: '<?= $this->session->flashdata('error') ?>', customClass: { confirmButton: 'btn btn-danger px-4' } });
    <?php endif; ?>
