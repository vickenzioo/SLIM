...
<head>
  <meta charset="utf-8">
  <title>SLIM | Network Product</title>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0" style="color: var(--text-dark); font-weight: 700;">Network Product Management</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item">
                <a href="<?= base_url('portofolio')?>" class="breadcrumb-home">Home</a></li>
              <li class="breadcrumb-item active">Network Product</li>
            </ol>
          </div>
        </div>
      </div>
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-submenu-compact" style="border-top: 3px solid var(--theme-yellow-primary);">
            ...
            <form id="mainFilterForm" action="<?= base_url('network_product') ?>" method="get">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-4">
                            <div class="input-group">
                                <input type="text" name="keyword" class="form-control" placeholder="Search Product Name..." value="<?= isset($keyword) ? $keyword : '' ?>">
                                <div class="input-group-append">
                                    <button class="btn btn-default">
                                        <i class="fas fa-search"></i>
                                    </button>
                                    <a href="<?= base_url('network_product') ?>" class="btn btn-secondary d-flex align-items-center">
                        ...
                        <table class="table table-striped table-bordered table-hover text-nowrap" style="width: 100%;">
                            <thead>
                                <tr class="bg-info">
                                    ...
                                    <?= render_th('Product Name', 'product_name', $opt_product_name, $selected_filters) ?>
                                    <?= render_th('Product SLA', 'product_sla', $opt_product_sla, $selected_filters) ?>
                            ...
                            <tbody>
    							<?php if(!empty($products)): ?>
    								...
    								<tr>
    									<td class="text-center align-middle">
    										<div class="dropdown">
    											...
    											<div class="dropdown-menu dropdown-menu-right shadow dropdown-operation-menu">
    												<button class="dropdown-item" type="button" onclick="window.location.href='<?= base_url('network_product/audit/'.$row['product_id']) ?>'">
    													<i class="fas fa-clipboard-list fa-fw text-primary mr-2"></i> Audit Trail
    												</button>

    												<?php if(isset($row['status']) && $row['status'] == 1): ?>
    													<button class="dropdown-item" type="button" onclick="editProduct('<?= $row['product_id'] ?>', '<?= htmlspecialchars($row['product_name']) ?>', '<?= $row['product_sla'] ?>')">
    														<i class="fas fa-edit fa-fw text-warning mr-2"></i> Edit Data
    													</button>
    												
    													<button class="dropdown-item" type="button" onclick="confirmDelete('<?= $row['network_product_id'] ?>')">
    														<i class="fas fa-power-off fa-fw text-danger mr-2"></i> Deactivate
    													</button>
    												<?php else: ?>
    													<button class="dropdown-item" type="button" onclick="confirmRestore('<?= $row['network_product_id'] ?>')">
    									...
    									<td class="align-middle"><?= !empty($row['product_sla']) ? $row['product_sla'] . '%' : '-' ?></td>
    									<td class="align-middle"><?= htmlspecialchars($row['network_name']) ?></td>
    									
    									<td class="text-center align-middle">
                                            <?php 
                                                // Menggunakan variabel $row['status'] sesuai konteks loop Network Product
...
<div class="modal fade modal-submenu-compact" id="modalForm" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?= base_url('network_product/save') ?>" method="post">
          <div class="modal-body">
            <input type="hidden" name="product_id" id="product_id">
            
            <div class="form-group">
                <label>Network Product Name</label>
                <input type="text" name="product_name" id="product_name" class="form-control" required placeholder="Enter Network Product Name">
            </div>

            <div class="form-group">
                <label>Product SLA (%)</label>
                <input type="text" name="product_sla" id="product_sla" class="form-control" required placeholder="Enter Product SLA">
            </div>
            ...
            <div class="form-group">
                ...
                <div class="form-text text-end" style="font-size: 0.85rem;">
                    <p style="text-align: right; font-size: 9px; color: #666; margin-top: 4px;">
...
<script>
    $(document).ready(function() {
        $('.select2').select2({
            ...
            allowClear: true,
            dropdownParent: $('#modalForm'), // Fix Select2 di Modal
            minimumResultsForSearch: Infinity // Untuk ilangin search di dropdown
        });
    ...
    function confirmExport() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "<?= base_url('network_product/export') ?>" + window.location.search;
            }
        })
    }
    ...
    function editProduct(id, name, sla) {      
        ...
        // Load data relasi
        $.ajax({
            url: "<?= base_url('network_product/get_related_networks/') ?>" + id,
            type: "GET",
            dataType: "JSON",
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= base_url('network_product/update_status') ?>",
                    type: "POST",
    ...
    $(document).on('input', '#product_sla, #product_name, #reason, input[name="keyword"], .filter-search-input, .swal2-popup input[type="text"], .swal2-popup textarea', function() {
        ...
        // 1. CEK: Apakah ini input untuk Product SLA?
        if (el.attr('id') === 'product_sla') {
            isSLA = true;
        } else if (el.hasClass('filter-search-input')) {
            // Ngintip data-key dari checkbox di bawahnya untuk form filter
            var columnKey = el.closest('.custom-filter-dropdown').find('input[type="checkbox"]').first().data('key');
            if (columnKey === 'product_sla') {
                isSLA = true;
            }
        }
        ...
        // 2. TENTUKAN REGEX
        if (isSLA) {
            // Khusus SLA: HANYA izinkan angka, titik (.), dan koma (,)
            forbiddenChars = /[^0-9.,]/g;
        } else {
            // Default: izinkan huruf, angka, spasi, titik, koma, strip, dan underscore
    ...
    $(document).on('paste', '#product_sla, #product_name, #reason, input[name="keyword"], .filter-search-input, .swal2-popup input[type="text"], .swal2-popup textarea', function(e) {
        var el = $(this);
        ...
        // 2. TENTUKAN REGEX SESUAI JENIS INPUT
        if (isSLA) {
            // Khusus SLA: HANYA izinkan angka, titik (.), dan koma (,)
            forbiddenChars = /[^0-9.,]/g;
        } else {
            // Default: izinkan huruf, angka, spasi, titik, koma, strip, underscore, dan kurung ()
            forbiddenChars = /[^a-zA-Z0-9\s.,_\-()]/g; 
        }

        // 3. EKSEKUSI BLOKIR JIKA MENGANDUNG KARAKTER TERLARANG
        if (forbiddenChars.test(pasteData)) {
            // Gagalkan proses paste secara total
            e.preventDefault();
