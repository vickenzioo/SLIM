...
class Network_model extends CI_Model {
    ...
    // 3. Count All
    public function count_all_networks($keyword = null, $filters = []) {
        if($keyword) {
            $this->db->group_start();
        ...
        return $this->db->count_all_results('tbl_apps_network');
    }

    // 4. Get Paginated (SORT BY NETWORK NAME ASC)
    public function get_networks_paginated($limit, $start, $keyword = null, $filters = []) {
        if($keyword) {
            $this->db->group_start();
            $this->db->like('network_name', $keyword);
            $this->db->group_end();
    ...
    public function get_dynamic_options($target_key, $current_filters = []) {
        ...
        $query = $this->db->get('tbl_apps_network');
    ...
    // 6. Get All for Export (SORT BY NETWORK NAME ASC)
    public function get_all_networks($keyword = null, $filters = []) {
        if($keyword) {
            $this->db->group_start();
            $this->db->like('network_name', $keyword);
            $this->db->group_end();
    ...
    // CRUD Helper
    public function insert_network($data) { return $this->db->insert('tbl_apps_network', $data); }
    public function update_network($id, $data) { $this->db->where('network_id', $id); return $this->db->update('tbl_apps_network', $data); }
    public function delete_network($id) { $this->db->where('network_id', $id); return $this->db->delete('tbl_apps_network'); }
    
    public function check_duplicate_network($name, $id = null) {
        $this->db->where('network_name', $name);
        if($id) { $this->db->where('network_id !=', $id); }
        return $this->db->get('tbl_apps_network')->num_rows() > 0;
    }
    
    public function get_by_id($id) {
        return $this->db->get_where('tbl_apps_network', ['network_id' => $id])->row_array();
    }

    public function update_network_status($id, $status) {
        $this->db->where('network_id', $id);
        return $this->db->update('tbl_apps_network', ['status' => $status]);
    }

    // (Opsional)
    public function get_audit_trail($id) {
        $this->db->where('table_name', 'tbl_apps_network'); 
