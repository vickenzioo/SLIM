...
<head>
  <meta charset="utf-8">
  <title>SLIM | Operating Software</title>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0" style="color: var(--text-dark); font-weight: 700;">Operating Software Management</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item">
                <a href="<?= base_url('portofolio')?>" class="breadcrumb-home">Home</a></li>
              <li class="breadcrumb-item active">Operating Software</li>
            </ol>
          </div>
        </div>
      </div>
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-submenu-compact" style="border-top: 3px solid var(--theme-yellow-primary);">
            ...
            <form id="mainFilterForm" action="<?= base_url('operating_software') ?>" method="get">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-4">
                            <div class="input-group">
                                <input type="text" name="keyword" class="form-control" placeholder="Search Operating Software Name..." value="<?= isset($keyword) ? $keyword : '' ?>">
                                <div class="input-group-append">
                                    <button class="btn btn-default">
                                        <i class="fas fa-search"></i>
                                    </button>
                                    <a href="<?= base_url('operating_software') ?>" class="btn btn-secondary d-flex align-items-center">
                        ...
                        <table class="table table-striped table-bordered table-hover text-nowrap" style="width: 100%;">
                            <thead>
                                <tr class="bg-info">
                                    ...
                                    <?= render_th('Operating Software Name', 'operating_software_name', $opt_operating_software_name, $selected_filters) ?>
    								<th style="width: 200px; text-align: center;">Status</th>
                                    
                                </tr>
                            </thead>
                            <tbody>
                                <?php if(!empty($operating_softwares)): ?>
                                    <?php 
                                        $start = $this->input->get('per_page');
                                        $no = $start ? $start + 1 : 1; 
                                    ?>
                                    <?php foreach($operating_softwares as $db): ?>
                                    <tr>
    									<td class="text-center align-middle">
    										<div class="dropdown">
    											...
    											<div class="dropdown-menu dropdown-menu-right shadow dropdown-operation-menu">
    												<button class="dropdown-item" type="button" onclick="window.location.href='<?= base_url('operating_software/audit/'.$db['operating_software_id']) ?>'">
    													<i class="fas fa-clipboard-list fa-fw text-primary mr-2"></i> Audit Trail
    												</button>

    												<?php if($db['status'] == 1): ?>
    													<button class="dropdown-item" type="button" onclick="editOpS(<?= $db['operating_software_id'] ?>, '<?= $db['operating_software_name'] ?>')">
    														<i class="fas fa-edit fa-fw text-warning mr-2"></i> Edit Data
    													</button>
    												
    													<button class="dropdown-item" type="button" onclick="confirmDelete(<?= $db['operating_software_id'] ?>)">
    														<i class="fas fa-power-off fa-fw text-danger mr-2"></i> Deactivate
    													</button>
    												<?php else: ?>
    													
    													<button class="dropdown-item" type="button" onclick="confirmRestore(<?= $db['operating_software_id'] ?>)">
                                        ...
                                        <td class="align-middle"><?= $db['operating_software_name'] ?></td>
                                        <td class="text-center align-middle">
    										<?php 
...
<div class="modal fade modal-submenu-compact" id="modalOperating_Software" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
       <div class="modal-header" style="background: linear-gradient(135deg, var(--theme-bg-yellow-light) 0%, var(--theme-bg-yellow-dark) 100%); color: var(--text-dark);">
        <h5 class="modal-title" id="modalTitle">Add Operating Software</h5>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <form action="<?= base_url('operating_software/save') ?>" method="post">
          <div class="modal-body">
            <input type="hidden" name="operating_software_id" id="operating_software_id">
            <div class="form-group">
                <label>Operating Software Name</label>
                <input type="text" name="operating_software_name" id="operating_software_name" class="form-control" required placeholder="Enter Operating Software Name">
            </div>
...
<script>
    ...
    function confirmExport() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "<?= base_url('operating_software/export') ?>" + window.location.search;
            }
        })
    }
    ...
    // --- STANDARD FUNCTIONS ---
    function clearForm() {
        $('#modalTitle').text('Add Operating Software');
        $('#operating_software_id').val('');
        $('#operating_software_name').val('');
        $('#reason').val(''); 
        $('#reason_container').hide(); 
        $('#modalOperating_Software').modal('show'); 
    }

    function editOpS(id, name) {
        $('#modalTitle').text('Edit Operating Software');
        $('#operating_software_id').val(id);
        $('#operating_software_name').val(name);
        $('#reason').val(''); 
                ...
                if (response.is_used) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "<?= base_url('operating_software/update_status') ?>",
                                type: "POST",
                                dataType: "JSON",
                                data: { 
                                    id: id, 
    ...
    $(document).on('input', '#operating_software_name, #reason, input[name="keyword"], .filter-search-input, .swal2-popup input[type="text"], .swal2-popup textarea', function() {
    ...
    $(document).on('paste', '#operating_software_name, #reason, input[name="keyword"], .filter-search-input, .swal2-popup input[type="text"], .swal2-popup textarea', function(e) {
