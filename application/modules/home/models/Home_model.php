...
class Home_model extends CI_Model {
    ...
    protected $_filter_map = [
        'app_status'         => "(CASE 
                                    WHEN a.status = 0 THEN 'Not Active' 
                                    WHEN ad.is_done IS NULL AND (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action IN ('RENEWAL', 'CHANGE OWNERSHIP')) = 0 THEN 'Drafting'
                                    WHEN ad.is_done IS NULL AND (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action = 'RENEWAL') > 0 THEN 'Renewal'
                                    ELSE 'Active' 
                                 END)",
        'status'             => "(CASE 
    ...
    private function _join_tables() {
        ...
        $this->db->join('(SELECT apps_id, 1 as is_done FROM tbl_apps_approval GROUP BY apps_id HAVING SUM(current) = 0 AND SUM(CASE WHEN user_role_id = 1 AND status = 1 THEN 1 ELSE 0 END) > 0) ad', 'ad.apps_id = a.apps_id', 'left');
    }
    ...
    private function _apply_rbac($user_id, $role_id) {
        ...
        elseif ($safe_role == 1) {
            // MATIKAN KODE DI BAWAH INI: Agar Role 1 (IT SLM) bisa melihat SEMUA data tanpa filter
    ...
    public function get_my_portfolio($user_id, $role_id, $keyword = null, $filters = [], $limit = 10, $start = 0) {
		...
		$this->db->select("(CASE 
			WHEN (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action = 'IMPORT') > 0 
			AND (SELECT COUNT(*) FROM tbl_apps_sla_history WHERE apps_id = a.apps_id) = 0 
			THEN 1 ELSE 0 
		END) as is_new_imported", FALSE);
		
		$this->db->select("(CASE 
                                WHEN a.status = 0 THEN 'Not Active'
                                WHEN MAX(ad.is_done) IS NULL AND (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action IN ('RENEWAL', 'CHANGE OWNERSHIP')) = 0 THEN 'Drafting'
                                WHEN MAX(ad.is_done) IS NULL AND (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action = 'RENEWAL') > 0 THEN 'Renewal'
                                ELSE 'Active' 
                             END) as app_status_label", FALSE);
        ...
        $this->db->order_by("(CASE 
                                WHEN a.status = 0 THEN 4
                                WHEN MAX(ad.is_done) IS NULL AND (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action IN ('RENEWAL', 'CHANGE OWNERSHIP')) = 0 THEN 1
                                WHEN MAX(ad.is_done) IS NULL AND (SELECT COUNT(*) FROM tbl_apps_audit_trail WHERE apps_id = a.apps_id AND action = 'RENEWAL') > 0 THEN 2
                                ELSE 3 
                             END)", "ASC", FALSE);
    ...
    public function get_dynamic_options($target_key, $user_id, $role_id, $current_filters = []) {
        ...
        // 1. Hapus filter untuk kolom ini sendiri agar daftar pilihannya tidak terkunci (hilang)
        $filters_to_apply = is_array($current_filters) ? $current_filters : [];
    ...
    public function save_apps_info($apps_id, $post_data, $is_submit = false, $role_id = 0, $remarks = null, $action_string = null) {
        ...
        $fields = [
            ...
            'lob_directorate', 'lob_subdirectorate', 'lob_group', 'lob_group_head', 'lob_department_head', 
            'it_subdirectorate',
        ...
        if (empty($apps_id) || $apps_id == 0) {
            ...
            $this->generate_initial_workflow_batch($apps_id, $user_id, $now, $is_submit, $remarks, $role_id);
        } else {
    ...
    private function generate_initial_workflow_batch($apps_id, $user_id, $now, $is_submit, $remarks, $role_id = 0) {
        if ($role_id == 0) $role_id = $this->session->userdata('role_id');

        $row_first = [
            'apps_id'      => $apps_id,
            'user_role_id' => $role_id, // Mencatat role pembuat secara dinamis (EA atau SLM)
            'created_at'   => $now,
            'created_by'   => $user_id,
            'modified_at'  => $now,
            'modified_by'  => $user_id,
        ...
        $this->db->insert('tbl_apps_approval', $row_first);
    ...
    private function _handle_inputter_logic($apps_id, $role_id, $user_id, $now, $is_submit, $remarks) {
        if (!$is_submit) {
            ...
            if ($role_id == 2) {
                ...
                // Jika DEV belum ada di history, lempar ke DEV (Submit Awal)
                $dev_exists = $this->db->where(['apps_id' => $apps_id, 'user_role_id' => 3])->count_all_results('tbl_apps_approval');
                if ($dev_exists == 0) {
                    $next_role = 3;
                }
            }
            ...
            if ($next_role > 0) {
                $this->_upsert_approval($apps_id, $next_role, [
                    ...
                    'created_by'  => $user_id,
                    'created_at'  => $now,
                    'modified_by' => $user_id, // <-- UBAH INI
                    'modified_at' => $now,     // <-- UBAH INI
                    'submit_date' => NULL,
                    'remarks'     => NULL
                ]);
            }
    ...
    public function advance_workflow($apps_id, $current_role_id, $action, $remarks = '') {
        ...
        $this->db->insert('tbl_apps_audit_trail', [
            'apps_id'    => $apps_id,
            'role_id'    => $role_id, 
            'action'     => !empty($action) ? $action : 'SUBMIT', // <--- PERBAIKAN DI SINI
    ...
    public function get_my_tasks($user_id, $role_id, $include_renewal = true) {
        $this->db->select('ap.approval_id, ap.apps_id, ap.user_role_id, ap.status, ap.current, ap.submit_date, ap.modified_by, ap.modified_at, ap.created_at AS ap_created_at');
        ...
        // --- LOGIKA PENGURUTAN PRIORITAS KATEGORI DIMATIKAN ---
        // Agar benar-benar diurutkan berdasarkan waktu masuk terbaru
        // -------------------------------------------------------
        ...
        foreach ($query as &$row) {
            $start_time = !empty($row['modified_at']) ? $row['modified_at'] : (!empty($row['ap_created_at']) ? $row['ap_created_at'] : $row['created_at']);
            
            // Simpan timestamp murni untuk acuan sorting nantinya
            $row['sort_time'] = strtotime($start_time); 
            ...
            if ($row['user_role_id'] == 2) { 
                ...
                // Mengecek apakah sudah ada role lain yang mensubmit (status = 1)
                $has_approved = $this->db->where('apps_id', $row['apps_id'])
                                         ->where('status', 1)
                                         ->count_all_results('tbl_apps_approval');

                if ($has_approved == 0) {
                    // Jika belum ada yang submit sama sekali, berarti ini murni Draft baru milik IT SLM
                    $row['task_status_label'] = 'Drafting';
                } else {
                    // Jika sudah ada (misal dari EA / IT Dev), berarti menunggu Final Review
        ...
        if ($role_id == 2 && $include_renewal) {
            ...
            foreach ($need_renewal_tasks as $nr) {
                ...
                // Simpan timestamp murni untuk acuan sorting nantinya
                $nr['sort_time'] = strtotime($start_time);
        ...
        // --- PENGURUTAN FINAL BERDASARKAN WAKTU TERBARU ---
        usort($query, function($a, $b) {
            return $b['sort_time'] <=> $a['sort_time']; // Descending (terbaru di atas)
        });
        // --------------------------------------------------
    ...
    public function get_timeline_data($apps_id) {
        // 1. Ambil nama role dari tabel master (agar penamaan dinamis dan valid)
        $roles_query = $this->db->where_in('role_id', [1, 2, 3])->get('tbl_role')->result_array();
        $role_names = [];
        foreach ($roles_query as $r) {
            $role_names[$r['role_id']] = $r['role_name'];
        }

        // 2. Ambil data approval yang ada di database untuk aplikasi ini
        $this->db->select('ap.user_role_id, ap.status, ap.current, ap.submit_date, ap.remarks, ap.modified_at, ap.modified_by, r.role_name');
        $this->db->from('tbl_apps_approval ap');
        $this->db->join('tbl_role r', 'r.role_id = ap.user_role_id', 'left');
        $this->db->where('ap.apps_id', $apps_id);
        $query = $this->db->get()->result_array();

        // 3. Petakan data berdasarkan user_role_id
        $timeline_map = [];
        foreach ($query as $row) {
            $timeline_map[$row['user_role_id']] = $row;
        }

        $result = [];
        // 4. SUSUNAN MUTLAK VISUAL: Role 2 (EA) -> Role 3 (DEV) -> Role 1 (SLM)
        $sequence = [2, 3, 1];

        foreach ($sequence as $r_id) {
            if (isset($timeline_map[$r_id])) {
                // Jika data ada di database (Normal flow), masukkan ke array
                $result[] = $timeline_map[$r_id];
            } else {
                // Jika data TIDAK ADA, buat data kosong abu-abu
                $result[] = [
                    'user_role_id' => $r_id,
                    'status'       => 0, // 0 = abu-abu / pending / inactive
                    'current'      => 0, // Bukan giliran saat ini
                    'submit_date'  => null,
                    'remarks'      => 'Waiting for Action', // PERBAIKAN: Ubah tulisan Bypassed menjadi ini
                    'modified_at'  => null,
                    'modified_by'  => null,
                    'role_name'    => isset($role_names[$r_id]) ? $role_names[$r_id] : 'Role'
                ];
            }
        }
    ...
    public function delete_app($apps_id) {
        ...
        // 1. Hapus data dari tabel relasi (Child) terlebih dahulu untuk menghindari Constraint Error
        $this->db->delete('tbl_apps_database', ['apps_id' => $apps_id]);
        $this->db->delete('tbl_apps_operating_software', ['apps_id' => $apps_id]);
        $this->db->delete('tbl_apps_server', ['apps_id' => $apps_id]); // Tambahkan ini karena kolom server sudah pindah tabel
        $this->db->delete('tbl_apps_approval', ['apps_id' => $apps_id]);
        $this->db->delete('tbl_apps_audit_trail', ['apps_id' => $apps_id]); // WAJIB: Agar tidak error 1451
        $this->db->delete('tbl_apps_sla_history', ['apps_id' => $apps_id]); // Tambahkan ini juga
    ...
    public function is_app_done($apps_id) {
        ...
        // Jika IT SLM sudah status 1, maka aplikasi dianggap DONE (SLA siap dibuat)
    ...
    public function process_renewal($apps_id, $user_id, $role_id) {
        $this->db->trans_start();
        $now = date('Y-m-d H:i:s'); // Ambil waktu saat ini
        ...
        // 2. Set EA (Role 2) sebagai aktif DAN catat waktu saat ini
        $this->db->where('apps_id', $apps_id);
        $this->db->where('user_role_id', 2);
        $this->db->update('tbl_apps_approval', [
            'current'     => 1,
            'modified_at' => $now,      // Catat waktu tekan tombol Yes
            'modified_by' => $user_id,
            'submit_date' => NULL       // Reset submit date karena kembali jadi Draft
        ]);
	...
	public function process_change_ownership($apps_id, $post_data, $user_id, $role_id) {
        $this->db->trans_start();
        $now = date('Y-m-d H:i:s');

        // 1. Ambil data lama untuk dibandingkan
        $old_data = $this->db->get_where('tbl_portofolio_apps_master', ['apps_id' => $apps_id])->row_array();

        // 2. Daftar field Ownership beserta labelnya untuk Remarks
        $fields_map = [
            'lob_directorate' => 'LOB Directorate',
            'lob_subdirectorate' => 'LOB Sub-Directorate',
            'lob_department_head' => 'LOB Department Head',
            'lob_group' => 'LOB Group',
            'lob_group_head' => 'LOB Group Head',
            'it_subdirectorate' => 'IT Sub-Directorate',
            'it_department_head' => 'IT Department Head',
            'it_support_group' => 'IT Support Group',
            'it_group_head' => 'IT Group Head',
            'it_support_divison' => 'IT Support Division',
        ...
        // 3. Bandingkan dan kumpulkan perubahannya
        foreach ($fields_map as $field => $label) {
            $new_val = isset($post_data[$field]) ? trim($post_data[$field]) : '';
            $old_val = isset($old_data[$field]) ? trim($old_data[$field]) : '';

            $update_data[$field] = $new_val;

            if ($new_val !== $old_val) {
                $old_text = ($old_val === '') ? '(kosong)' : $old_val;
                $new_text = ($new_val === '') ? '(kosong)' : $new_val;
                $changes[] = "- {$label} : '{$old_text}' -> '{$new_text}'";
            }
        }

        // 4. Susun Remarks
        if (empty($changes)) {
            $remarks = "Change Ownership diajukan, namun tidak ada data yang diubah.";
        } else {
            $remarks = "Perubahan Kepemilikan (Change Ownership):\n" . implode("\n", $changes);
        }

        // 5. Update Master Data
        $this->db->where('apps_id', $apps_id)->update('tbl_portofolio_apps_master', $update_data);

        // 6. Reset Workflow (Kembali ke Role 2 EA)
        $this->db->where('apps_id', $apps_id)->update('tbl_apps_approval', ['status' => 0, 'current' => 0]);
        $this->db->where(['apps_id' => $apps_id, 'user_role_id' => 2])->update('tbl_apps_approval', [
            'current' => 1,
            'modified_at' => $now,
            'modified_by' => $user_id,
            'submit_date' => NULL,
        ...
        $this->db->insert('tbl_apps_audit_trail', [
            ...
            'action'     => 'RENEWAL',
            'remarks'    => 'Aplikasi masuk masa perpanjangan (Renewal).',
            'created_at' => date('Y-m-d H:i:s', strtotime($now . ' +1 second'))
        ]);
    ...
    public function insert_audit_trail($apps_id, $role_id, $action, $remarks) {
        $data = [
            'apps_id'    => $apps_id,
            'role_id'    => $role_id,
            'action'     => !empty($action) ? $action : 'SUBMIT', // <--- PERBAIKAN DI SINI
        ...
        return $this->db->insert('tbl_apps_audit_trail', $data);
    }
}
